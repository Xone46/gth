<template>
  <div class="container">
    <div id="nav">
      <router-link
        to="/interventions"
        v-if="this.$route.path === '/interventions'"
      >
        Les Interventions
      </router-link>
    </div>
    ...
    <table class="table table-bordered table-striped table-wrapper">
      ...
      <tbody id="myTable">
        <tr v-for="row in rowsInterventions" :key="row._id">
          ...
          <td>{{ row.repere }}</td>
          <td>
            {{ row.adresse }} / {{ row.codepostal }} - {{ row.ville }} /
            {{ row.pays }}
          </td>
          <td>{{ row.metier }}</td>
          <td>{{ row.agence }}</td>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
      <button type="button" class="btn btn-success btn-sm" v-on:click="cree()">
        Crée un Intervention
      </button>
      <button
        type="button"
        ...
        v-on:click="modifier()"
      >
        Modifie un Intervention
      </button>
      <button type="button" class="btn btn-success btn-sm" v-on:click="voir()">
