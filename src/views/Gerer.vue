<template>
  <div class="container">
    <div id="nav">
      <router-link to="/gerer" v-if="this.$route.path === '/gerer'">
  ...
  methods: {
    ...
    searchCompte() {
      if (this.filter != "") {
        ...
        const resultat = this.rowsUser.find(
          (el) =>
            el.nom.toLowerCase() === this.searchTerm ||
            el.identite.toLowerCase() === this.searchTerm
        );
        if (resultat) {
